...
// ============================================================
// BOT PROFILER V4 / PRE-BUY WINDOW FORENSICS
// ------------------------------------------------------------
...
const PROFILE_OUT_DIR = (process.env.PROFILE_OUT_DIR || "profiler-output-v4").trim();
const BIRDEYE_API_KEY = (process.env.BIRDEYE_API_KEY || "").trim();
const BIRDEYE_ENABLED = BIRDEYE_API_KEY.length > 0;

// ============================================================
// V4 PRE-BUY WINDOW CONFIG
// ============================================================
// V4 focuses on what happens immediately BEFORE the target BUY. It does not
// need the expensive create->buy scan unless PROFILE_DEEP_MINTS is explicitly
// set above zero. The default BAT sets PROFILE_DEEP_MINTS=0.
const PROFILE_PREBUY_MINTS = clampInt(process.env.PROFILE_PREBUY_MINTS, 80, 5, 300);
const PROFILE_PREBUY_MAX_SIGS = clampInt(process.env.PROFILE_PREBUY_MAX_SIGS, 1200, 50, 5000);
const PROFILE_PREBUY_PAGE_SIZE = clampInt(process.env.PROFILE_PREBUY_PAGE_SIZE, 100, 10, 500);
const PROFILE_PREBUY_WINDOWS_SEC = parseWindows(process.env.PROFILE_PREBUY_WINDOWS_SEC || "10,30,60,180");
const PROFILE_PREBUY_MAX_WINDOW_SEC = Math.max(...PROFILE_PREBUY_WINDOWS_SEC);
...
type PreBuyEvent = {
  ...
  leadSec: number;
  tokenAmount: number;
  solAmountEstimate: number | null;
  priceProxySolPerToken: number | null;
};
...
type WindowFlow = {
  ...
  buySolEstimate: number;
  sellSolEstimate: number;
  netSolEstimate: number;
  buyTokenAmount: number;
  sellTokenAmount: number;
  buySellTxRatio: number | null;
  buySellSolRatio: number | null;
  priceMomentumPct: number | null;
};

type PreBuyWalletDetail = {
  wallet: string;
  closestBuyLeadSec: number | null;
  earliestBuyLeadSec: number | null;
  buyCount: number;
  sellCount: number;
  buySolEstimate: number;
  sellSolEstimate: number;
  netTokenAmount: number;
  buyWithin: Record<string, number>;
};
...
type PreBuyMint = {
  ...
  targetTime: number;
  targetSlot: number;
  targetOutcome: MintPosition["outcome"];
  targetEstimatedRoiPct: number | null;
  scannedPriorSignatures: number;
  events: PreBuyEvent[];
  windows: Record<string, WindowFlow>;
  walletDetails: PreBuyWalletDetail[];
  buyRateAcceleration30s: number | null;
  buySolAcceleration30s: number | null;
};

type PreBuyWalletStat = {
  wallet: string;
  appearances180: number;
  appearances60: number;
  appearances30: number;
  appearances10: number;
  wins180: number;
  losses180: number;
  closedWinRate180: number | null;
  medianClosestLeadSec: number | null;
  totalBuySolEstimate: number;
  netBuyerMints: number;
};
...
function parseWindows(raw: string): number[] {
  ...
  const unique = [...new Set(values.map((x) => Math.max(1, Math.trunc(x))))].sort((a, b) => a - b);
  return unique.length ? unique : [10, 30, 60, 180];
}
...
// ============================================================
// V4 PRE-BUY WINDOW SCAN
// ============================================================

function walletSolDeltaForTx(tx: any, wallet: string): number | null {
  const keys = txAccountKeys(tx);
...
async function getPriorMintSignaturesWithinWindow(
  mint: string,
  targetSignature: string,
  targetTime: number,
): Promise<SigInfo[]> {
  const cutoff = targetTime - PROFILE_PREBUY_MAX_WINDOW_SEC;
  const out: SigInfo[] = [];
  let before: string | undefined = targetSignature;
  let scanned = 0;

  while (scanned < PROFILE_PREBUY_MAX_SIGS) {
    const limit = Math.min(PROFILE_PREBUY_PAGE_SIZE, PROFILE_PREBUY_MAX_SIGS - scanned);
    const opts: any = { limit, commitment: "confirmed" };
    if (before) opts.before = before;
    const page = await rpcCall<any[]>("getSignaturesForAddress", [mint, opts]);
    if (!page.length) break;
    scanned += page.length;
    ...
    for (const x of page) {
      const blockTime = x.blockTime == null ? null : Number(x.blockTime);
      if (blockTime != null && blockTime < cutoff) {
        reachedCutoff = true;
        continue;
      }
      if (!x.err && blockTime != null && blockTime <= targetTime) {
        out.push({ signature: String(x.signature), slot: Number(x.slot), blockTime, err: x.err });
      }
    }

    if (reachedCutoff || page.length < limit) break;
...
function summarizeWindow(events: PreBuyEvent[], seconds: number): WindowFlow {
  const xs = events.filter((e) => e.leadSec >= 0 && e.leadSec <= seconds);
  const buys = xs.filter((e) => e.direction === "BUY");
  const sells = xs.filter((e) => e.direction === "SELL");
  const buySol = buys.reduce((s, e) => s + (e.solAmountEstimate ?? 0), 0);
  const sellSol = sells.reduce((s, e) => s + (e.solAmountEstimate ?? 0), 0);
  const prices = xs
    .filter((e) => e.priceProxySolPerToken != null && e.priceProxySolPerToken! > 0)
    .sort((a, b) => a.blockTime - b.blockTime || a.slot - b.slot)
    .map((e) => e.priceProxySolPerToken!);
  const priceMomentumPct = prices.length >= 2 && prices[0] > 0
  ...
  return {
    ...
    buyTx: buys.length,
    sellTx: sells.length,
    uniqueBuyers: new Set(buys.map((e) => e.wallet)).size,
    uniqueSellers: new Set(sells.map((e) => e.wallet)).size,
    buySolEstimate: buySol,
    sellSolEstimate: sellSol,
    netSolEstimate: buySol - sellSol,
    buyTokenAmount: buys.reduce((s, e) => s + e.tokenAmount, 0),
    sellTokenAmount: sells.reduce((s, e) => s + e.tokenAmount, 0),
    buySellTxRatio: sells.length > 0 ? buys.length / sells.length : (buys.length > 0 ? Infinity : null),
    buySellSolRatio: sellSol > 0 ? buySol / sellSol : (buySol > 0 ? Infinity : null),
    priceMomentumPct,
  };
}

async function scanPreBuyMint(position: MintPosition): Promise<PreBuyMint> {
  const firstBuy = position.firstBuy!;
  const sigs = await getPriorMintSignaturesWithinWindow(position.mint, firstBuy.signature, firstBuy.blockTime);
  const txMap = await getTransactions(sigs.map((x) => x.signature));
  const events: PreBuyEvent[] = [];

  for (const si of sigs) {
    const tx = txMap.get(si.signature);
    if (!tx || tx?.meta?.err || !venueFromTx(tx)) continue;
    const ownerDeltas = tokenOwnerDeltasForMint(tx.meta, position.mint);
    const signers = txSigners(tx);
    for (const wallet of signers) {
      if (!wallet || wallet === TARGET_WALLET) continue;
      const d = ownerDeltas.get(wallet);
      if (!d || d.raw === 0n) continue;
      const direction: "BUY" | "SELL" = d.raw > 0n ? "BUY" : "SELL";
      const tokenAmount = Math.abs(rawToUi(d.raw, d.decimals));
      const solDelta = walletSolDeltaForTx(tx, wallet);
      const solAmountEstimate = solDelta == null
        ? null
        : direction === "BUY"
          ? Math.max(0, -solDelta)
          : Math.max(0, solDelta);
      const priceProxySolPerToken = solAmountEstimate != null && solAmountEstimate > 0 && tokenAmount > 0
        ? solAmountEstimate / tokenAmount
        : null;
      events.push({
        wallet,
        direction,
        ...
        slot: si.slot,
        blockTime: si.blockTime!,
        leadSec: Math.max(0, firstBuy.blockTime - si.blockTime!),
        tokenAmount,
        solAmountEstimate,
        priceProxySolPerToken,
      });
    }
  }

  events.sort((a, b) => a.blockTime - b.blockTime || a.slot - b.slot || a.signature.localeCompare(b.signature));

  const windows: Record<string, WindowFlow> = {};
  for (const seconds of PROFILE_PREBUY_WINDOWS_SEC) windows[String(seconds)] = summarizeWindow(events, seconds);

  const byWallet = new Map<string, PreBuyEvent[]>();
  for (const e of events) {
    if (!byWallet.has(e.wallet)) byWallet.set(e.wallet, []);
    byWallet.get(e.wallet)!.push(e);
  }
  const walletDetails: PreBuyWalletDetail[] = [...byWallet.entries()].map(([wallet, xs]) => {
    const buys = xs.filter((e) => e.direction === "BUY");
    const sells = xs.filter((e) => e.direction === "SELL");
    const buyWithin: Record<string, number> = {};
    for (const seconds of PROFILE_PREBUY_WINDOWS_SEC) {
      buyWithin[String(seconds)] = buys.filter((e) => e.leadSec <= seconds).length;
    }
    return {
      wallet,
      closestBuyLeadSec: buys.length ? Math.min(...buys.map((e) => e.leadSec)) : null,
      earliestBuyLeadSec: buys.length ? Math.max(...buys.map((e) => e.leadSec)) : null,
      buyCount: buys.length,
      sellCount: sells.length,
      buySolEstimate: buys.reduce((s, e) => s + (e.solAmountEstimate ?? 0), 0),
      sellSolEstimate: sells.reduce((s, e) => s + (e.solAmountEstimate ?? 0), 0),
      netTokenAmount: xs.reduce((s, e) => s + (e.direction === "BUY" ? e.tokenAmount : -e.tokenAmount), 0),
      buyWithin,
    };
  }).sort((a, b) => (a.closestBuyLeadSec ?? Infinity) - (b.closestBuyLeadSec ?? Infinity));

  const w30 = windows["30"] || summarizeWindow(events, 30);
  const w180 = windows[String(PROFILE_PREBUY_MAX_WINDOW_SEC)] || summarizeWindow(events, PROFILE_PREBUY_MAX_WINDOW_SEC);
  const prevSeconds = Math.max(1, PROFILE_PREBUY_MAX_WINDOW_SEC - 30);
  const prevBuyTx = Math.max(0, w180.buyTx - w30.buyTx);
  const recentRate = w30.buyTx / 30;
  const prevRate = prevBuyTx / prevSeconds;
  const buyRateAcceleration30s = prevRate > 0 ? recentRate / prevRate : (recentRate > 0 ? Infinity : null);
  const prevBuySol = Math.max(0, w180.buySolEstimate - w30.buySolEstimate);
  const recentSolRate = w30.buySolEstimate / 30;
  const prevSolRate = prevBuySol / prevSeconds;
  const buySolAcceleration30s = prevSolRate > 0 ? recentSolRate / prevSolRate : (recentSolRate > 0 ? Infinity : null);

  return {
    mint: position.mint,
    targetSignature: firstBuy.signature,
    targetTime: firstBuy.blockTime,
    targetSlot: firstBuy.slot,
    targetOutcome: position.outcome,
    targetEstimatedRoiPct: position.estimatedRoiPct,
    scannedPriorSignatures: sigs.length,
    events,
    windows,
    walletDetails,
    buyRateAcceleration30s,
    buySolAcceleration30s,
  };
}

function buildPreBuyHeuristics(pre: PreBuyMint[]) {
  const stats = new Map<string, {
    a180: number; a60: number; a30: number; a10: number;
    wins: number; losses: number; leads: number[]; buySol: number; netBuyer: number;
  }>();

  for (const p of pre) {
    for (const d of p.walletDetails) {
      const has180 = (d.buyWithin["180"] ?? d.buyWithin[String(PROFILE_PREBUY_MAX_WINDOW_SEC)] ?? 0) > 0;
      const has60 = (d.buyWithin["60"] ?? 0) > 0;
      const has30 = (d.buyWithin["30"] ?? 0) > 0;
      const has10 = (d.buyWithin["10"] ?? 0) > 0;
      if (!has180 && !has60 && !has30 && !has10) continue;
      const s = stats.get(d.wallet) || { a180: 0, a60: 0, a30: 0, a10: 0, wins: 0, losses: 0, leads: [], buySol: 0, netBuyer: 0 };
      if (has180) s.a180++;
      if (has60) s.a60++;
      if (has30) s.a30++;
      if (has10) s.a10++;
      if (p.targetOutcome === "win" && has180) s.wins++;
      if (p.targetOutcome === "loss" && has180) s.losses++;
      if (d.closestBuyLeadSec != null) s.leads.push(d.closestBuyLeadSec);
      s.buySol += d.buySolEstimate;
      if (d.netTokenAmount > 0) s.netBuyer++;
      stats.set(d.wallet, s);
    }
  }

  const walletStats: PreBuyWalletStat[] = [...stats.entries()].map(([wallet, s]) => ({
    wallet,
    appearances180: s.a180,
    appearances60: s.a60,
    appearances30: s.a30,
    appearances10: s.a10,
    wins180: s.wins,
    losses180: s.losses,
    closedWinRate180: (s.wins + s.losses) > 0 ? s.wins / (s.wins + s.losses) : null,
    medianClosestLeadSec: median(s.leads),
    totalBuySolEstimate: s.buySol,
    netBuyerMints: s.netBuyer,
  })).sort((a, b) =>
    b.appearances30 - a.appearances30 ||
    b.appearances60 - a.appearances60 ||
    b.appearances10 - a.appearances10 ||
    b.appearances180 - a.appearances180 ||
    (a.medianClosestLeadSec ?? Infinity) - (b.medianClosestLeadSec ?? Infinity) ||
    a.wallet.localeCompare(b.wallet)
  );

  const finiteAccel = pre.map((p) => p.buyRateAcceleration30s).filter((x): x is number => x != null && Number.isFinite(x));
  const finiteSolAccel = pre.map((p) => p.buySolAcceleration30s).filter((x): x is number => x != null && Number.isFinite(x));
  const ratios60 = pre.map((p) => p.windows["60"]?.buySellTxRatio ?? null).filter((x): x is number => x != null && Number.isFinite(x));
  const net60 = pre.map((p) => p.windows["60"]?.netSolEstimate ?? null).filter((x): x is number => x != null && Number.isFinite(x));
  const findings: string[] = [];
  const best = walletStats[0];
  if (best && best.appearances30 >= Math.max(3, Math.ceil(pre.length * 0.08))) {
    findings.push(`Recurring immediate pre-buy wallet: ${short(best.wallet)} bought within 30s before target on ${best.appearances30}/${pre.length} profiled mints; median closest lead ${fmtSec(best.medianClosestLeadSec)}.`);
  }
  const medAccel = median(finiteAccel);
  const medSolAccel = median(finiteSolAccel);
  const medRatio60 = median(ratios60);
  if (medAccel != null && medAccel >= 2) findings.push(`Order-flow acceleration signature: median 30s BUY transaction rate is ${medAccel.toFixed(2)}x the preceding window.`);
  if (medSolAccel != null && medSolAccel >= 2) findings.push(`Capital acceleration signature: estimated BUY SOL rate in the final 30s is ${medSolAccel.toFixed(2)}x the preceding window.`);
  if (medRatio60 != null && medRatio60 >= 2) findings.push(`Buy-pressure signature: median 60s BUY/SELL transaction ratio is ${medRatio60.toFixed(2)}x before target entries.`);
  if (!findings.length) findings.push("No single 10/30/60/180s pre-buy trigger dominates the current sample; inspect wallet recurrence and flow CSVs together.");

  return {
    walletStats,
    medianBuyRateAcceleration30s: medAccel,
    medianBuySolAcceleration30s: medSolAccel,
    medianBuySellTxRatio60s: medRatio60,
    medianNetSol60s: median(net60),
    findings,
  };
}

function writeV4Reports(
  positions: MintPosition[],
  trades: Trade[],
  pre: PreBuyMint[],
  preHeur: ReturnType<typeof buildPreBuyHeuristics>,
  baseHeur: ReturnType<typeof strategyHeuristics>,
) {
  ensureDir(PROFILE_OUT_DIR);
  const stamp = new Date().toISOString().replace(/[:.]/g, "-");
  const base = path.join(PROFILE_OUT_DIR, `${TARGET_WALLET.slice(0, 8)}-${stamp}`);
  const summaryPath = `${base}-summary.txt`;
  const jsonPath = `${base}.json`;
  const walletCsvPath = `${base}-prebuy-wallets.csv`;
  const mintCsvPath = `${base}-prebuy-mints.csv`;
  const eventCsvPath = `${base}-prebuy-events.csv`;

  fs.writeFileSync(jsonPath, safeJson({
    generatedAt: new Date().toISOString(),
    targetWallet: TARGET_WALLET,
    config: {
      PROFILE_TX_LIMIT,
      PROFILE_PREBUY_MINTS,
      PROFILE_PREBUY_WINDOWS_SEC,
      PROFILE_PREBUY_MAX_SIGS,
      PROFILE_PREBUY_PAGE_SIZE,
      PROFILE_RPC_BATCH,
      PROFILE_BATCH_DELAY_MS,
    },
    preBuyHeuristics: preHeur,
    executionHeuristics: baseHeur,
    preBuyMints: pre,
    positions,
    trades,
  }));
  ...
  const walletLines = [
    "wallet,within180,within60,within30,within10,wins180,losses180,closed_win_rate180,median_closest_lead_sec,total_buy_sol_estimate,net_buyer_mints",
    ...preHeur.walletStats.map((s) => [
      s.wallet, s.appearances180, s.appearances60, s.appearances30, s.appearances10,
      s.wins180, s.losses180,
      s.closedWinRate180 == null ? "" : (s.closedWinRate180 * 100).toFixed(2),
      s.medianClosestLeadSec ?? "", s.totalBuySolEstimate, s.netBuyerMints,
    ].map(csvEscape).join(",")),
  ];
  fs.writeFileSync(walletCsvPath, walletLines.join("\n"));

  const mintHeaders = ["mint", "target_time", "outcome", "roi_est_pct", "scanned_prior_sigs", "accel_30s_tx", "accel_30s_sol"];
  for (const w of PROFILE_PREBUY_WINDOWS_SEC) mintHeaders.push(`buytx_${w}s`, `selltx_${w}s`, `buyers_${w}s`, `sellers_${w}s`, `buysol_${w}s`, `sellsol_${w}s`, `netsol_${w}s`, `momentum_${w}s_pct`);
  const mintLines = [mintHeaders.join(",")];
  for (const p of pre) {
    const row: any[] = [p.mint, new Date(p.targetTime * 1000).toISOString(), p.targetOutcome, p.targetEstimatedRoiPct ?? "", p.scannedPriorSignatures, p.buyRateAcceleration30s ?? "", p.buySolAcceleration30s ?? ""];
    for (const w of PROFILE_PREBUY_WINDOWS_SEC) {
      const f = p.windows[String(w)];
      row.push(f?.buyTx ?? 0, f?.sellTx ?? 0, f?.uniqueBuyers ?? 0, f?.uniqueSellers ?? 0, f?.buySolEstimate ?? 0, f?.sellSolEstimate ?? 0, f?.netSolEstimate ?? 0, f?.priceMomentumPct ?? "");
    }
    mintLines.push(row.map(csvEscape).join(","));
  }
  fs.writeFileSync(mintCsvPath, mintLines.join("\n"));

  const eventLines = ["mint,target_time,event_time,lead_sec,wallet,direction,token_amount,sol_estimate,price_proxy_sol_per_token,signature"];
  for (const p of pre) {
    for (const e of p.events) {
      eventLines.push([p.mint, new Date(p.targetTime * 1000).toISOString(), new Date(e.blockTime * 1000).toISOString(), e.leadSec, e.wallet, e.direction, e.tokenAmount, e.solAmountEstimate ?? "", e.priceProxySolPerToken ?? "", e.signature].map(csvEscape).join(","));
    }
  }
  fs.writeFileSync(eventCsvPath, eventLines.join("\n"));

  const lines: string[] = [];
  lines.push("============================================================");
  lines.push("BOT PROFILER V4 / PRE-BUY WINDOW FINGERPRINT");
  lines.push("============================================================");
  lines.push(`Target: ${TARGET_WALLET}`);
  lines.push(`Generated: ${new Date().toISOString()}`);
  lines.push(`Target Pump/PumpSwap trades parsed: ${trades.length}`);
  lines.push(`Unique target mints: ${positions.length}`);
  lines.push(`Pre-buy mints profiled: ${pre.length}`);
  lines.push(`Windows: ${PROFILE_PREBUY_WINDOWS_SEC.join(" / ")} sec`);
  lines.push("");
  lines.push(`Median target hold: ${fmtSec(baseHeur.medianHoldSec)}`);
  lines.push(`Median 30s BUY tx acceleration: ${preHeur.medianBuyRateAcceleration30s == null ? "n/a" : `${preHeur.medianBuyRateAcceleration30s.toFixed(2)}x`}`);
  lines.push(`Median 30s BUY SOL acceleration: ${preHeur.medianBuySolAcceleration30s == null ? "n/a" : `${preHeur.medianBuySolAcceleration30s.toFixed(2)}x`}`);
  lines.push(`Median 60s BUY/SELL tx ratio: ${preHeur.medianBuySellTxRatio60s == null ? "n/a" : `${preHeur.medianBuySellTxRatio60s.toFixed(2)}x`}`);
  lines.push(`Median 60s net SOL estimate: ${preHeur.medianNetSol60s == null ? "n/a" : preHeur.medianNetSol60s.toFixed(6)}`);
  lines.push("");
  lines.push("LIKELY PRE-BUY SIGNALS");
  preHeur.findings.forEach((x) => lines.push(`- ${x}`));
  lines.push("");
  lines.push("TOP WALLETS BUYING BEFORE TARGET");
  lines.push("wallet                                            180s  60s  30s  10s   W/L   median lead   est buy SOL");
  preHeur.walletStats.slice(0, 30).forEach((s) => {
    lines.push(`${s.wallet}  ${String(s.appearances180).padStart(4)} ${String(s.appearances60).padStart(4)} ${String(s.appearances30).padStart(4)} ${String(s.appearances10).padStart(4)}  ${String(s.wins180).padStart(3)}/${String(s.losses180).padEnd(3)}  ${fmtSec(s.medianClosestLeadSec).padStart(12)}  ${s.totalBuySolEstimate.toFixed(4)}`);
  });
  lines.push("");
  lines.push("EXECUTION FINGERPRINT");
  baseHeur.findings.forEach((x) => lines.push(`- ${x}`));
  lines.push("");
  lines.push("IMPORTANT: repeated public wallets or correlated flow are signals, not proof of insider status, coordination, or common ownership.");
  fs.writeFileSync(summaryPath, lines.join("\n"));

  return { summaryPath, jsonPath, walletCsvPath, mintCsvPath, eventCsvPath };
}
...
async function main() {
  ...
  console.log("============================================================");
  console.log("       BOT PROFILER V4 / PRE-BUY WINDOW FORENSICS");
  console.log("                    READ-ONLY MODE");
  console.log("============================================================");
  console.log(`Target wallet:           ${TARGET_WALLET}`);
  console.log(`Target history:          ${PROFILE_TX_LIMIT} tx`);
  console.log(`Pre-buy mints:           ${PROFILE_PREBUY_MINTS}`);
  console.log(`Windows:                 ${PROFILE_PREBUY_WINDOWS_SEC.join(", ")} sec`);
  console.log(`Max prior sigs / mint:   ${PROFILE_PREBUY_MAX_SIGS}`);
  console.log(`Pre-buy page size:       ${PROFILE_PREBUY_PAGE_SIZE}`);
  console.log(`Optional deep mints:     ${PROFILE_DEEP_MINTS}`);
  ...
  console.log(`[1/7] Fetching last ${PROFILE_TX_LIMIT} target-wallet signatures...`);
  ...
  console.log("[2/7] Fetching target transaction bodies...");
  const txMap = await getTransactions(sigInfos.map((x) => x.signature));
  console.log(`[OK] ${txMap.size} transaction bodies.`);

  console.log("[3/7] Parsing Pump.fun / PumpSwap BUY and SELL activity...");
  ...
  const candidatePositions = positions.filter((p) => p.firstBuy).slice(0, PROFILE_PREBUY_MINTS);
  const pre: PreBuyMint[] = [];
  console.log(`[4/7] Scanning ${candidatePositions.length} pre-buy windows...`);
  for (let i = 0; i < candidatePositions.length; i++) {
    const p = candidatePositions[i];
    console.log(`\n[WINDOW ${i + 1}/${candidatePositions.length}] ${p.mint}`);
    try {
      const r = await scanPreBuyMint(p);
      pre.push(r);
      const w10 = r.windows["10"];
      const w30 = r.windows["30"];
      const w60 = r.windows["60"];
      const top = r.walletDetails.filter((x) => (x.buyWithin["60"] || 0) > 0).slice(0, 5);
      console.log(`  outcome:        ${r.targetOutcome}${r.targetEstimatedRoiPct == null ? "" : ` (${r.targetEstimatedRoiPct >= 0 ? "+" : ""}${r.targetEstimatedRoiPct.toFixed(1)}% est.)`}`);
      console.log(`  10s flow:       buys=${w10?.buyTx ?? 0} sells=${w10?.sellTx ?? 0} buyers=${w10?.uniqueBuyers ?? 0}`);
      console.log(`  30s flow:       buys=${w30?.buyTx ?? 0} sells=${w30?.sellTx ?? 0} buyers=${w30?.uniqueBuyers ?? 0} accel=${r.buyRateAcceleration30s == null ? "n/a" : Number.isFinite(r.buyRateAcceleration30s) ? `${r.buyRateAcceleration30s.toFixed(2)}x` : "INF"}`);
      console.log(`  60s flow:       buys=${w60?.buyTx ?? 0} sells=${w60?.sellTx ?? 0} netSOL=${(w60?.netSolEstimate ?? 0).toFixed(4)}`);
      console.log(`  nearest buyers: ${top.map((x) => `${short(x.wallet)}@${fmtSec(x.closestBuyLeadSec)}`).join(", ") || "none"}`);
    } catch (e: any) {
      console.log(`  [WARN] pre-buy scan failed: ${e?.message || e}`);
    }
  }

  const deep: DeepMint[] = [];
  if (PROFILE_DEEP_MINTS > 0) {
    const deepPositions = positions.filter((p) => p.firstBuy).slice(0, PROFILE_DEEP_MINTS);
    console.log(`\n[5/7] Optional create/early-buyer deep scan for ${deepPositions.length} mints...`);
    for (let i = 0; i < deepPositions.length; i++) {
      const p = deepPositions[i];
      console.log(`\n[DEEP ${i + 1}/${deepPositions.length}] ${p.mint}`);
      try {
        ...
        deep.push(d);
        console.log(`  buyer rank: ${d.targetBuyerRank ?? "n/a"}; create->buy=${d.createToTargetMs == null ? "n/a" : fmtSec(d.createToTargetMs / 1000)}`);
      } catch (e: any) {
        console.log(`  [WARN] deep scan failed: ${e?.message || e}`);
      }
    }
  } else {
    console.log("\n[5/7] Create/early-buyer deep scan skipped (V4 fast mode). V3 already covers this.");
  }

  console.log("[6/7] Building pre-buy wallet + order-flow fingerprints...");
  const preHeur = buildPreBuyHeuristics(pre);
  const baseHeur = strategyHeuristics(positions, deep, trades);

  console.log("[7/7] Writing V4 reports...");
  const files = writeV4Reports(positions, trades, pre, preHeur, baseHeur);

  console.log("\n============================================================");
  console.log("               PRE-BUY STRATEGY FINGERPRINT");
  console.log("============================================================");
  console.log(`Profiled pre-buy mints:         ${pre.length}`);
  console.log(`Median target hold:             ${fmtSec(baseHeur.medianHoldSec)}`);
  console.log(`Median 30s BUY tx acceleration: ${preHeur.medianBuyRateAcceleration30s == null ? "n/a" : `${preHeur.medianBuyRateAcceleration30s.toFixed(2)}x`}`);
  console.log(`Median 30s BUY SOL acceleration:${preHeur.medianBuySolAcceleration30s == null ? " n/a" : ` ${preHeur.medianBuySolAcceleration30s.toFixed(2)}x`}`);
  console.log(`Median 60s BUY/SELL tx ratio:   ${preHeur.medianBuySellTxRatio60s == null ? "n/a" : `${preHeur.medianBuySellTxRatio60s.toFixed(2)}x`}`);
  console.log("\nLikely pre-buy signals:");
  preHeur.findings.forEach((x) => console.log(`  - ${x}`));
  console.log("\nTop wallets immediately before target:");
  preHeur.walletStats.slice(0, 20).forEach((s) => {
    const wr = s.closedWinRate180 == null ? "n/a" : `${(s.closedWinRate180 * 100).toFixed(0)}%`;
    console.log(`  ${s.wallet}  180=${s.appearances180} 60=${s.appearances60} 30=${s.appearances30} 10=${s.appearances10}  W/L=${s.wins180}/${s.losses180}  WR=${wr}  lead=${fmtSec(s.medianClosestLeadSec)}`);
  });
  console.log("\nReports:");
  console.log(`  ${files.summaryPath}`);
  console.log(`  ${files.jsonPath}`);
  console.log(`  ${files.walletCsvPath}`);
  console.log(`  ${files.mintCsvPath}`);
  console.log(`  ${files.eventCsvPath}`);
