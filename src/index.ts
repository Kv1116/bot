...
function safeCopyEntryAllowed(
  ...
  return {
    allowed: true,
    reason: `entry premium ${(premiumBps / 100).toFixed(2)}% is within limit`,
    premiumBps,
  };
}
...
async function copySell(
  ...
  // Arbitrary proportional SELL. Nothing is hard-coded to 50%.
  // 30% target SELL -> 30% bot SELL, etc. Each following SELL uses
  // the remaining tracked position.
  if (
