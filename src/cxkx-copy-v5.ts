...
// Emergency Pump.fun SELL recovery. Error 6003 is the Pump bonding-curve
// TooLittleSolReceived / slippage failure. A hard-stop must not sit exposed
// waiting for the normal price monitor to trigger again, so explicit 6003
// failures are rebuilt immediately from fresh on-chain reserves with wider
// slippage and a higher priority fee. We only retry after a CONFIRMED on-chain
// failure, never after an ambiguous timeout, which avoids accidentally selling
// manual holdings if the first transaction actually landed.
const EMERGENCY_SELL_RETRY_ENABLED =
  String(
    process.env.EMERGENCY_SELL_RETRY_ENABLED || "true",
  ).toLowerCase() !== "false";

const EMERGENCY_SELL_MAX_RETRIES =
  Math.max(
    0,
    Math.min(
      5,
      Number(
        process.env.EMERGENCY_SELL_MAX_RETRIES || "3",
      ) || 3,
    ),
  );

const EMERGENCY_SELL_SLIPPAGES =
  String(
    process.env.EMERGENCY_SELL_SLIPPAGES || "15,25,40",
  )
    .split(",")
    .map(value => Number(value.trim()))
    .filter(value => Number.isFinite(value) && value > 0)
    .map(value => Math.max(SELL_SLIPPAGE, value));

const EMERGENCY_SELL_PRIORITY_FEE_MICROLAMPORTS =
  Math.max(
    SELL_PRIORITY_FEE_MICROLAMPORTS,
    Number(
      process.env.EMERGENCY_SELL_PRIORITY_FEE_MICROLAMPORTS || "350000",
    ) || 350000,
  );

const EMERGENCY_SELL_RETRY_DELAY_MS =
  Math.max(
    0,
    Math.min(
      1000,
      Number(
        process.env.EMERGENCY_SELL_RETRY_DELAY_MS || "35",
      ) || 35,
    ),
  );
...
// Never chase a BUY that sat in our local queue for too long. Cxkx's edge is
// strongest when we react close to his transaction; once a signal is stale,
// the copy-trading premium/rug risk rises sharply.
const MAX_BUY_SIGNAL_AGE_MS =
  Math.max(
    100,
    Math.min(
      10_000,
      Number(
        process.env.MAX_BUY_SIGNAL_AGE_MS || "700",
      ) || 700,
    ),
  );

// Session-only circuit breaker. After several losing autonomous exits in a
// row, stop opening new positions for a short period. This does not touch an
// already-open position and resets after a profitable autonomous exit.
// Positions inherited from a previous process are not counted toward this
// session streak.
const SAFE_COPY_CONSECUTIVE_LOSS_LIMIT =
  Math.max(
    0,
    Math.min(
      10,
      Number(
        process.env.SAFE_COPY_CONSECUTIVE_LOSS_LIMIT || "3",
      ) || 3,
    ),
  );

const SAFE_COPY_LOSS_PAUSE_MS =
  Math.max(
    0,
    Math.min(
      60 * 60 * 1000,
      Number(
        process.env.SAFE_COPY_LOSS_PAUSE_MS || "300000",
      ) || 300000,
    ),
  );
...
const sessionOpenedMints =
  new Set<string>();

let safeCopyConsecutiveLosses =
  0;

let safeCopyPauseUntilMs =
  0;

function recordSafeCopySessionExit(
  mintString: string,
  pnlBps: number | null,
  reason: string,
): void {
  if (
    pnlBps === null ||
    !sessionOpenedMints.has(mintString)
  ) {
    return;
  }

  if (pnlBps < 0) {
    safeCopyConsecutiveLosses += 1;

    status(
      `SAFE COPY session loss streak: ${safeCopyConsecutiveLosses}/${SAFE_COPY_CONSECUTIVE_LOSS_LIMIT || "off"} after ${reason} (${(pnlBps / 100).toFixed(2)}%).`,
      "warning",
    );

    if (
      SAFE_COPY_CONSECUTIVE_LOSS_LIMIT > 0 &&
      safeCopyConsecutiveLosses >= SAFE_COPY_CONSECUTIVE_LOSS_LIMIT &&
      SAFE_COPY_LOSS_PAUSE_MS > 0
    ) {
      safeCopyPauseUntilMs =
        Date.now() +
        SAFE_COPY_LOSS_PAUSE_MS;

      status(
        `SAFE COPY circuit breaker armed: new BUYs paused for ${(SAFE_COPY_LOSS_PAUSE_MS / 1000).toFixed(0)}s after ${safeCopyConsecutiveLosses} consecutive losses.`,
        "error",
      );

      safeCopyConsecutiveLosses =
        0;
    }
  ...
  if (safeCopyConsecutiveLosses > 0) {
    status(
      `SAFE COPY loss streak reset after profitable exit (${(pnlBps / 100).toFixed(2)}%).`,
      "success",
    );
  }
...
function safeCopyBuyPauseRemainingMs(): number {
  return Math.max(
    0,
    safeCopyPauseUntilMs - Date.now(),
  );
}
...
function isConfirmedPumpSellSlippageFailure(
  error: unknown,
): boolean {
  const text =
  ...
  // Only retry a transaction when the chain explicitly rejected it.
  // Custom 6003 on the Pump bonding curve is TooLittleSolReceived.
  return (
    text.includes("failed on-chain") &&
    (
      text.includes('"Custom":6003') ||
      text.includes('Custom:6003') ||
      text.includes('Custom: 6003') ||
      text.includes("TooLittleSolReceived") ||
      text.includes("Too little SOL received")
    )
  );
}

async function emergencyRetryPumpSell(
  mint: PublicKey,
  wallet: Keypair,
  tokenAmount: BN,
  tokenProgram: PublicKey,
  ...
  if (
    !EMERGENCY_SELL_RETRY_ENABLED ||
    !isConfirmedPumpSellSlippageFailure(
      firstError,
    )
  ) {
    throw firstError;
  ...
  const retryCount =
    Math.min(
      ...
      Math.max(
        1,
        EMERGENCY_SELL_SLIPPAGES.length,
      ),
    );
  ...
  for (
    ...
    const slippage =
      EMERGENCY_SELL_SLIPPAGES[attempt] ??
      EMERGENCY_SELL_SLIPPAGES[
        EMERGENCY_SELL_SLIPPAGES.length - 1
      ] ??
      Math.max(
        SELL_SLIPPAGE,
        15,
    ...
    if (
      EMERGENCY_SELL_RETRY_DELAY_MS > 0
    ) {
      await sleep(
        EMERGENCY_SELL_RETRY_DELAY_MS,
      );
    }

    status(
      `EMERGENCY SELL retry ${attempt + 1}/${retryCount}: fresh curve, ${slippage}% slippage, priority ${EMERGENCY_SELL_PRIORITY_FEE_MICROLAMPORTS}.`,
      "warning",
    );
    ...
    try {
      ...
      // Graduation can happen between quote and retry. If the bonding curve is
      // gone/complete, attempt the canonical PumpSwap pool immediately.
      if (
        !freshCurve.exists ||
        freshCurve.complete ||
        !freshCurve.bondingCurve ||
        !freshCurve.accountInfo
      ) {
        status(
          "Emergency SELL detected completed/unavailable bonding curve; trying PumpSwap.",
          "warning",
        );

        const pumpSwapSignature =
        ...
        if (!pumpSwapSignature) {
          throw new Error(
            "Emergency PumpSwap SELL returned no signature.",
          );
        }

        await waitForSignatureProcessed(
          pumpSwapSignature,
          `EMERGENCY PUMPSWAP SELL #${attempt + 1}`,
        );

        status(
          `EMERGENCY SELL recovered through PumpSwap on attempt ${attempt + 1}.`,
          "success",
        );

        return pumpSwapSignature;
      }
        ...
        await copyPumpBondingCurveSell(
          ...
          tokenProgram,
          {
            ...
            priorityFeeMicrolamports:
              EMERGENCY_SELL_PRIORITY_FEE_MICROLAMPORTS,
            senderTipLamports:
              SELL_SENDER_TIP_LAMPORTS,
            label:
              `PUMP SELL EMERGENCY #${attempt + 1}`,
            forceFreshCurve:
              false,
          },
        );
      ...
      if (!signature) {
        throw new Error(
          "Emergency Pump.fun SELL returned no signature.",
        );
      }

      await waitForSignatureProcessed(
        signature,
        `EMERGENCY PUMP SELL #${attempt + 1}`,
      );

      status(
        `EMERGENCY SELL recovered on attempt ${attempt + 1} with ${slippage}% slippage.`,
        "success",
      );
      ...
      console.error(
        `Emergency SELL attempt ${attempt + 1} failed:`,
        error,
      );

      // A confirmed slippage rejection is safe to retry. Anything else may be
      // ambiguous (for example a timeout), so stop instead of risking a second
      // successful sale against unrelated/manual holdings of the same mint.
      if (
        !isConfirmedPumpSellSlippageFailure(
          error,
        )
    ...
    try {
      try {
        ...
        if (
          ...
          status(
            "Pump.fun SELL hit confirmed slippage error 6003; starting immediate emergency recovery instead of waiting for another price tick.",
            "warning",
          );

          signature =
            await emergencyRetryPumpSell(
              mint,
              getBotWallet(),
              botAmount,
              tracker.tokenProgram,
...
function printMoneyBanner(): void {
  ...
  console.log(
    paint(
      center("CXKX COPY V5 • STALE-SIGNAL GUARD + LOSS CIRCUIT BREAKER"),
      ANSI.bold,
      ANSI.yellow,
    ),
  );
...
interface PumpBondingCurveSellOptions {
  slippage?: number;
  priorityFeeMicrolamports?: number;
  senderTipLamports?: number;
  label?: string;
  forceFreshCurve?: boolean;
}
...
async function copyPumpBondingCurveSell(
  ...
  prefetchedTokenProgram?: PublicKey,
  options: PumpBondingCurveSellOptions = {},
): Promise<string | null> {
  const sellLabel =
    options.label ||
    "PUMP SELL";
  ...
  const sellSlippage =
    Math.max(
      SELL_SLIPPAGE,
      Number(
        options.slippage ?? SELL_SLIPPAGE,
      ) || SELL_SLIPPAGE,
    );

  console.log(
    `Building Pump.fun SELL... slippage=${sellSlippage}%`,
  );
...
async function copySell(
  ...
  if (
    ...
    try {
      ...
      // V5: target-wallet SELLs use the same emergency 6003 recovery as
      // autonomous hard-stop / take-profit exits. V3 only recovered
      // autonomous exits, so a copied target SELL could repeatedly fail with
      // TooLittleSolReceived while the strict execution queue backed up.
      if (
        executionVenue === "Pump.fun" &&
        isConfirmedPumpSellSlippageFailure(
          error,
        ...
        status(
          "TARGET COPY SELL hit confirmed Pump 6003; rebuilding from fresh reserves and entering emergency recovery now.",
          "warning",
        );

        const retryTokenProgram =
          fastBalanceTokenProgram ||
          await getMintTokenProgram(
            mint,
          );
        ...
        status(
          "TARGET COPY SELL recovered successfully; tracked position can now be closed.",
          "success",
        );
      } else {
        throw error;
  ...
  if (
    ...
    if (SAFE_COPY_MODE) {
      const signalAgeMs =
        Math.max(
          0,
          Date.now() - analyzed.observedAtMs,
        );

      if (
        signalAgeMs >
        MAX_BUY_SIGNAL_AGE_MS
      ) {
        status(
          `SAFE COPY BUY skipped: signal is stale (${signalAgeMs} ms > ${MAX_BUY_SIGNAL_AGE_MS} ms max).`,
          "warning",
        );
      ...
      const pauseRemainingMs =
        safeCopyBuyPauseRemainingMs();

      if (pauseRemainingMs > 0) {
        status(
          `SAFE COPY BUY skipped: loss circuit breaker active for another ${(pauseRemainingMs / 1000).toFixed(1)}s.`,
          "warning",
        );
...
async function monitor(): Promise<void> {
  ...
  kv("EMERGENCY_SELL_RETRY", EMERGENCY_SELL_RETRY_ENABLED);
  kv("EMERGENCY_SELL_MAX_RETRIES", EMERGENCY_SELL_MAX_RETRIES);
  kv("EMERGENCY_SELL_SLIPPAGES", EMERGENCY_SELL_SLIPPAGES.join(",") || "none");
  kv("EMERGENCY_SELL_PRIORITY", EMERGENCY_SELL_PRIORITY_FEE_MICROLAMPORTS);
  kv("MAX_BUY_SIGNAL_AGE_MS", MAX_BUY_SIGNAL_AGE_MS);
  kv("SAFE_COPY_LOSS_STREAK_LIMIT", SAFE_COPY_CONSECUTIVE_LOSS_LIMIT);
  kv("SAFE_COPY_LOSS_PAUSE_MS", SAFE_COPY_LOSS_PAUSE_MS);
