...
function safeCopyEntryAllowed(
  ...
  if (
    ...
    return {
      allowed: false,
      reason: "SAFE COPY accepts only direct Pump.fun BUY TradeEvents",
      premiumBps: null,
    };
  }
  ...
  if (!trade.pumpSolAmount) {
    return {
      allowed: false,
      reason: "signal BUY has no Pump SOL amount",
      premiumBps: null,
    };
  }
  ...
  const targetBuySol =
    Number(
      trade.pumpSolAmount.toString(10),
    ) / LAMPORTS_PER_SOL;

  const premiumBps =
    directPumpEntryPremiumBps(
      trade,
    );

  let signalState =
    targetSignalStates.get(
      trade.mint,
    );

  // A stale signal starts a fresh sequence for this mint.
  if (
    signalState &&
  ...
  if (trade.pumpRealSolReserves) {
    const realReserveSol =
      Number(
        trade.pumpRealSolReserves.toString(10),
      ) / LAMPORTS_PER_SOL;
  ...
  if (
    ...
    safeCopyOpenPositionCount() >=
      SAFE_COPY_MAX_OPEN_POSITIONS
  ) {
    return {
      allowed: false,
  ...
  const maxBps =
    Math.round(
      SAFE_COPY_MAX_ENTRY_PREMIUM_PERCENT *
        100,
    );

  // A large first BUY is the leader signal. If it has already moved the curve
  // too much, do not chase it; keep the mint armed and wait for the target to
  // add again. This matches the observed 3 SOL -> ~1 SOL -> ~1 SOL pattern.
  if (
    signalState.buyCount === 1 &&
  ...
  // Small BUYs are useful only as confirmation when a meaningful leader BUY was
  // seen first. This stops us from copying random ~1 SOL taps in old positions.
  if (
  ...
  return {
    ...
    reason: isConfirmation
      ...
      : `direct leader BUY ${targetBuySol.toFixed(3)} SOL; entry premium ${(premiumBps / 100).toFixed(2)}% is within limit`,
    premiumBps,
  };
}
...
function printMoneyBanner(): void {
  ...
  console.log(
    paint(
      center("CONFIRMATION ENTRY • FAST FULL EXIT"),
      ANSI.bold,
      ANSI.yellow,
    ),
  );
...
async function getTargetTransaction(
  ...
  // Standard-WebSocket fallback only. The Enhanced stream normally bypasses
  // this function entirely. Keep the retries tight to minimize copy latency.
  // Free-tier Helius: one immediate read usually succeeds after onLogs.
  // If indexing is a little behind, retry only a few times. Firing many
  // getParsedTransaction calls in the same slot causes 429 responses and is
  // slower overall because the SDK then backs off for ~500 ms.
  const delays =
    [
      0,
      35,
