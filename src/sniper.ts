...
// ============================================================
// The watched wallet is the signal source. We only react to meaningful early
// Pump.fun BUYs instead of trying to predict winners from random launches.
const SIGNAL_MIN_TARGET_BUY_SOL =
  Math.max(
    0,
    Number(
      process.env.SIGNAL_MIN_TARGET_BUY_SOL || "1.5",
    ) || 1.5,
  );
...
function safeCopyEntryAllowed(
  ...
  if (
    ...
    return {
      allowed: false,
      reason: "SAFE COPY allows only one copied BUY per mint",
      premiumBps: null,
    };
  }
  ...
  if (premiumBps > maxBps) {
    return {
      allowed: false,
      reason: `entry premium ${(premiumBps / 100).toFixed(2)}% exceeds ${SAFE_COPY_MAX_ENTRY_PREMIUM_PERCENT.toFixed(2)}% limit`,
      premiumBps,
    };
  }
  ...
  return {
    allowed: true,
    reason: `target BUY ${targetBuySol.toFixed(3)} SOL; entry premium ${(premiumBps / 100).toFixed(2)}% is within limit`,
    premiumBps,
  };
}
...
function printMoneyBanner(): void {
  ...
  console.log(
    paint(
      center("TRACKED POSITIONS • PROPORTIONAL SELL"),
      ANSI.bold,
      ANSI.yellow,
    ),
  );
