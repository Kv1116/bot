...
const PROFILE_PREBUY_POSITIONS = clampInt(process.env.PROFILE_PREBUY_POSITIONS, 80, 10, 250);
const PROFILE_PREBUY_WINDOW_SEC = clampInt(process.env.PROFILE_PREBUY_WINDOW_SEC, 180, 30, 900);
const PROFILE_PREBUY_SIG_LIMIT = clampInt(process.env.PROFILE_PREBUY_SIG_LIMIT, 1500, 100, 10000);
const PROFILE_PREBUY_PAGE_SIZE = clampInt(process.env.PROFILE_PREBUY_PAGE_SIZE, 250, 50, 1000);
const PROFILE_BASELINE_WALLETS = clampInt(process.env.PROFILE_BASELINE_WALLETS, 12, 0, 50);
const PROFILE_BASELINE_SIG_LIMIT = clampInt(process.env.PROFILE_BASELINE_SIG_LIMIT, 1000, 100, 5000);
...
// ============================================================
// V4 PRE-BUY WINDOW TYPES
// ============================================================

type PreBuyEvent = {
  targetMint: string;
  targetBuySignature: string;
  targetBuyTime: number;
...
type WindowMetric = {
  ...
  buySellEventRatio: number | null;
  proxyMomentumPct: number | null;
};
...
type PreBuyMint = {
  ...
  topBuyers180: Array<{ wallet: string; closestLeadSec: number; buyEvents: number; sellEvents: number; netRaw: string; estimatedBuySol: number }>;
};
...
type PreBuyWalletStat = {
  ...
  flats: number;
  openOrUnknown: number;
  medianClosestBuyLeadSec: number | null;
  medianEstimatedBuySol: number | null;
  netPositiveWindowCount: number;
  targetHitRate: number;
  baselineTx1h: number | null;
  baselineTx24h: number | null;
  baseline24hLowerBound: boolean;
};

function getWalletLamportDelta(tx: any, wallet: string): number | null {
  const keys = txAccountKeys(tx);
...
async function getPriorMintSignaturesInWindow(
  mint: string,
  targetSignature: string,
  targetTime: number,
): Promise<{ sigs: SigInfo[]; scanned: number; truncated: boolean }> {
  const cutoff = targetTime - PROFILE_PREBUY_WINDOW_SEC - 3;
  const out: SigInfo[] = [];
  let before: string | undefined = targetSignature;
  let scanned = 0;
  let truncated = false;

  while (scanned < PROFILE_PREBUY_SIG_LIMIT) {
    const pageLimit = Math.min(PROFILE_PREBUY_PAGE_SIZE, PROFILE_PREBUY_SIG_LIMIT - scanned);
    const opts: any = { limit: pageLimit, commitment: "confirmed" };
    if (before) opts.before = before;
    const page = await rpcCall<any[]>("getSignaturesForAddress", [mint, opts]);
    if (!page.length) break;
    ...
    for (const x of page) {
      const bt = x.blockTime == null ? null : Number(x.blockTime);
      if (bt != null && bt < cutoff) reachedCutoff = true;
      if (x.err || bt == null) continue;
      if (bt >= targetTime) continue;
    ...
    if (reachedCutoff || page.length < pageLimit) break;
    before = String(page[page.length - 1].signature);
    if (scanned >= PROFILE_PREBUY_SIG_LIMIT) {
      truncated = true;
      break;
    }
    if (PROFILE_BATCH_DELAY_MS) await sleep(PROFILE_BATCH_DELAY_MS);
  ...
  out.sort((a, b) => (a.blockTime || 0) - (b.blockTime || 0) || a.slot - b.slot);
  return { sigs: out, scanned, truncated };
}

function extractPreBuyEvents(
  mint: string,
  target: MintPosition,
  targetBuy: Trade,
  sigInfos: SigInfo[],
  ...
  const sigMeta = new Map(sigInfos.map((s) => [s.signature, s]));

  for (const [signature, tx] of txMap.entries()) {
    if (!tx?.meta || tx.meta.err) continue;
    const venue = venueFromTx(tx);
    if (!venue) continue;
    const blockTime = Number(tx.blockTime ?? sigMeta.get(signature)?.blockTime ?? 0);
    const slot = Number(tx.slot ?? sigMeta.get(signature)?.slot ?? 0);
    if (!blockTime || blockTime >= targetBuy.blockTime) continue;
    const leadSec = targetBuy.blockTime - blockTime;
    if (leadSec < 0 || leadSec > PROFILE_PREBUY_WINDOW_SEC + 3) continue;

    const deltas = tokenOwnerDeltasForMint(tx.meta, mint);
    const signers = txSigners(tx);
    const signerSet = new Set(signers);

    let candidates = [...deltas.entries()].filter(([owner, d]) => d.raw !== 0n && signerSet.has(owner));
    if (!candidates.length) {
      // Fallback for unusual transaction layouts where token owner is not exposed
      // as a signer in jsonParsed. Program/system accounts are excluded.
      const excluded = new Set([PUMP_PROGRAM, PUMPSWAP_PROGRAM, COMPUTE_BUDGET_PROGRAM, SYSTEM_PROGRAM, WSOL_MINT]);
      candidates = [...deltas.entries()].filter(([owner, d]) => d.raw !== 0n && !excluded.has(owner));
    }

    for (const [wallet, d] of candidates) {
      ...
      const solDelta = getWalletLamportDelta(tx, wallet);
      let estimatedSol: number | null = null;
      if (solDelta != null) {
        estimatedSol = direction === "BUY" ? Math.max(0, -solDelta) : Math.max(0, solDelta);
        if (estimatedSol === 0) estimatedSol = null;
      }
      const priceProxySolPerToken = estimatedSol != null && estimatedSol > 0 && tokenAmount > 0
        ? estimatedSol / tokenAmount
        : null;
      events.push({
        targetMint: mint,
        targetBuySignature: targetBuy.signature,
        targetBuyTime: targetBuy.blockTime,
...
function proxyMomentum(events: PreBuyEvent[]): number | null {
  const vals = events
    .filter((e) => e.priceProxySolPerToken != null && e.priceProxySolPerToken! > 0)
    .sort((a, b) => a.blockTime - b.blockTime)
    .map((e) => e.priceProxySolPerToken!);
  if (vals.length < 2) return null;
  const group = Math.max(1, Math.floor(vals.length / 3));
  const first = median(vals.slice(0, group));
  const last = median(vals.slice(-group));
  if (first == null || last == null || first <= 0) return null;
  return ((last / first) - 1) * 100;
}

function metricForWindow(events: PreBuyEvent[], sec: number): WindowMetric {
  const xs = events.filter((e) => e.leadSec <= sec + 0.001);
  const buys = xs.filter((e) => e.direction === "BUY");
  const sells = xs.filter((e) => e.direction === "SELL");
  const estimatedBuySol = buys.reduce((s, e) => s + (e.estimatedSol || 0), 0);
  const estimatedSellSol = sells.reduce((s, e) => s + (e.estimatedSol || 0), 0);
  return {
    seconds: sec,
    ...
    estimatedBuySol,
    estimatedSellSol,
    estimatedNetSol: estimatedBuySol - estimatedSellSol,
    buySellEventRatio: sells.length > 0 ? buys.length / sells.length : (buys.length > 0 ? Infinity : null),
    proxyMomentumPct: proxyMomentum(xs),
  };
}

function buildTopBuyers180(events: PreBuyEvent[]) {
  const byWallet = new Map<string, { buyEvents: number; sellEvents: number; netRaw: bigint; closestLeadSec: number; estimatedBuySol: number }>();
  for (const e of events) {
    const s = byWallet.get(e.wallet) || { buyEvents: 0, sellEvents: 0, netRaw: 0n, closestLeadSec: Infinity, estimatedBuySol: 0 };
    if (e.direction === "BUY") {
      s.buyEvents++;
      s.closestLeadSec = Math.min(s.closestLeadSec, e.leadSec);
      s.estimatedBuySol += e.estimatedSol || 0;
    } else {
      s.sellEvents++;
    }
    s.netRaw += e.rawDelta;
    byWallet.set(e.wallet, s);
  }
  return [...byWallet.entries()]
    .filter(([, s]) => s.buyEvents > 0)
    .map(([wallet, s]) => ({ wallet, closestLeadSec: s.closestLeadSec, buyEvents: s.buyEvents, sellEvents: s.sellEvents, netRaw: s.netRaw.toString(), estimatedBuySol: s.estimatedBuySol }))
    .sort((a, b) => a.closestLeadSec - b.closestLeadSec || b.buyEvents - a.buyEvents)
    .slice(0, 20);
}

async function scanOnePreBuy(position: MintPosition): Promise<{ summary: PreBuyMint; events: PreBuyEvent[] }> {
  const targetBuy = position.firstBuy!;
  const prior = await getPriorMintSignaturesInWindow(position.mint, targetBuy.signature, targetBuy.blockTime);
  const txMap = await getTransactions(prior.sigs.map((s) => s.signature));
  const events = extractPreBuyEvents(position.mint, position, targetBuy, prior.sigs, txMap);
  const windows: Record<string, WindowMetric> = {};
  for (const sec of [10, 30, 60, 180]) windows[String(sec)] = metricForWindow(events, sec);
  return {
    summary: {
      mint: position.mint,
      targetBuySignature: targetBuy.signature,
      targetBuyTime: targetBuy.blockTime,
      targetBuySlot: targetBuy.slot,
      targetOutcome: position.outcome,
      targetEstimatedRoiPct: position.estimatedRoiPct,
      targetHoldSec: position.holdSeconds,
      priorSignaturesScanned: prior.scanned,
      truncated: prior.truncated,
      windows,
      topBuyers180: buildTopBuyers180(events),
    },
    events,
  };
}

function buildPreBuyWalletStats(mints: PreBuyMint[], events: PreBuyEvent[]): PreBuyWalletStat[] {
  const byTarget = new Map<string, PreBuyEvent[]>();
  for (const e of events) {
    const key = e.targetBuySignature;
    if (!byTarget.has(key)) byTarget.set(key, []);
    byTarget.get(key)!.push(e);
  }
  const outcomeByTarget = new Map(mints.map((m) => [m.targetBuySignature, m.targetOutcome]));
  const agg = new Map<string, {
    a180: number; a60: number; a30: number; a10: number;
    wins: number; losses: number; flats: number; open: number;
    leads: number[]; buySols: number[]; netPositive: number;
  }>();

  for (const [targetSig, xs] of byTarget.entries()) {
    const byWallet = new Map<string, PreBuyEvent[]>();
    for (const e of xs) {
      if (!byWallet.has(e.wallet)) byWallet.set(e.wallet, []);
      byWallet.get(e.wallet)!.push(e);
    }
    for (const [wallet, wx] of byWallet.entries()) {
      const buys = wx.filter((e) => e.direction === "BUY");
      if (!buys.length) continue;
      const s = agg.get(wallet) || { a180: 0, a60: 0, a30: 0, a10: 0, wins: 0, losses: 0, flats: 0, open: 0, leads: [], buySols: [], netPositive: 0 };
      s.a180++;
      if (buys.some((e) => e.leadSec <= 60)) s.a60++;
      if (buys.some((e) => e.leadSec <= 30)) s.a30++;
      if (buys.some((e) => e.leadSec <= 10)) s.a10++;
      s.leads.push(Math.min(...buys.map((e) => e.leadSec)));
      const sol = buys.reduce((sum, e) => sum + (e.estimatedSol || 0), 0);
      if (sol > 0) s.buySols.push(sol);
      const netRaw = wx.reduce((sum, e) => sum + e.rawDelta, 0n);
      if (netRaw > 0n) s.netPositive++;
      const outcome = outcomeByTarget.get(targetSig) || "unknown";
      if (outcome === "win") s.wins++;
      else if (outcome === "loss") s.losses++;
      else if (outcome === "flat") s.flats++;
      else s.open++;
      agg.set(wallet, s);
    }
  }

  const denom = Math.max(1, mints.length);
  return [...agg.entries()].map(([wallet, s]) => ({
    wallet,
    ...
    losses: s.losses,
    flats: s.flats,
    openOrUnknown: s.open,
    medianClosestBuyLeadSec: median(s.leads),
    medianEstimatedBuySol: median(s.buySols),
    netPositiveWindowCount: s.netPositive,
    targetHitRate: s.a180 / denom,
    baselineTx1h: null,
    baselineTx24h: null,
    baseline24hLowerBound: false,
  })).sort((a, b) =>
    b.appearances30 - a.appearances30 ||
    b.appearances60 - a.appearances60 ||
    b.appearances180 - a.appearances180 ||
    (a.medianClosestBuyLeadSec ?? Infinity) - (b.medianClosestBuyLeadSec ?? Infinity) ||
    a.wallet.localeCompare(b.wallet)
  );
}

async function enrichBaseline(stats: PreBuyWalletStat[]): Promise<void> {
  if (PROFILE_BASELINE_WALLETS <= 0) return;
  const now = Math.floor(Date.now() / 1000);
  const top = stats.slice(0, PROFILE_BASELINE_WALLETS);
  console.log(`[5/6] Baseline-checking ${top.length} recurring wallets (recent signature activity)...`);
  for (let i = 0; i < top.length; i++) {
    const s = top[i];
    try {
      const sigs = await getSignatures(s.wallet, PROFILE_BASELINE_SIG_LIMIT);
      const oneHour = sigs.filter((x) => (x.blockTime || 0) >= now - 3600).length;
      const day = sigs.filter((x) => (x.blockTime || 0) >= now - 86400).length;
      const oldest = sigs.length ? (sigs[sigs.length - 1].blockTime || 0) : 0;
      s.baselineTx1h = oneHour;
      s.baselineTx24h = day;
      s.baseline24hLowerBound = sigs.length >= PROFILE_BASELINE_SIG_LIMIT && oldest >= now - 86400;
      console.log(`  [${i + 1}/${top.length}] ${short(s.wallet)}  1h=${oneHour}  24h=${day}${s.baseline24hLowerBound ? "+" : ""}`);
    } catch (e: any) {
      console.log(`  [WARN] baseline ${short(s.wallet)}: ${e?.message || e}`);
    }
    if (PROFILE_BATCH_DELAY_MS) await sleep(PROFILE_BATCH_DELAY_MS);
  }
}

function medianWindow(mints: PreBuyMint[], sec: number, key: keyof WindowMetric): number | null {
  const vals = mints.map((m) => m.windows[String(sec)]?.[key]).filter((v): v is number => typeof v === "number" && Number.isFinite(v));
  return median(vals);
}
...
function writeV4Reports(mints: PreBuyMint[], events: PreBuyEvent[], stats: PreBuyWalletStat[], targetTrades: Trade[]) {
  ensureDir(PROFILE_OUT_DIR);
  const stamp = new Date().toISOString().replace(/[:.]/g, "-");
  const base = path.join(PROFILE_OUT_DIR, `${TARGET_WALLET.slice(0, 8)}-${stamp}`);
  const jsonPath = `${base}.json`;
  ...
  fs.writeFileSync(jsonPath, safeJson({
    ...
    config: {
      PROFILE_TX_LIMIT,
      PROFILE_PREBUY_POSITIONS,
      PROFILE_PREBUY_WINDOW_SEC,
      PROFILE_PREBUY_SIG_LIMIT,
      PROFILE_BASELINE_WALLETS,
    },
    mints,
    walletStats: stats,
    events,
  ...
  const walletLines = [
    ["wallet","appear180","appear60","appear30","appear10","wins","losses","flats","open_unknown","median_closest_buy_lead_sec","median_est_buy_sol","net_positive_window_count","target_hit_rate","baseline_tx_1h","baseline_tx_24h","baseline_24h_lower_bound"].join(",")
  ];
  for (const s of stats) walletLines.push([
    s.wallet,s.appearances180,s.appearances60,s.appearances30,s.appearances10,s.wins,s.losses,s.flats,s.openOrUnknown,
    s.medianClosestBuyLeadSec ?? "",s.medianEstimatedBuySol ?? "",s.netPositiveWindowCount,s.targetHitRate,
    s.baselineTx1h ?? "",s.baselineTx24h ?? "",s.baseline24hLowerBound,
  ].map(csvEscape).join(","));
  fs.writeFileSync(walletCsvPath, walletLines.join("\n"));

  const mintHeaders = ["mint","target_buy_time","target_buy_signature","outcome","roi_pct","hold_sec","prior_sigs_scanned","truncated"];
  for (const sec of [10,30,60,180]) mintHeaders.push(`buy_${sec}s`,`sell_${sec}s`,`uniq_buy_${sec}s`,`uniq_sell_${sec}s`,`buy_sol_${sec}s`,`sell_sol_${sec}s`,`net_sol_${sec}s`,`event_ratio_${sec}s`,`proxy_momentum_${sec}s`);
  const mintLines = [mintHeaders.join(",")];
  for (const m of mints) {
    const row: any[] = [m.mint,new Date(m.targetBuyTime*1000).toISOString(),m.targetBuySignature,m.targetOutcome,m.targetEstimatedRoiPct ?? "",m.targetHoldSec ?? "",m.priorSignaturesScanned,m.truncated];
    for (const sec of [10,30,60,180]) {
      const w=m.windows[String(sec)];
      row.push(w.buyEvents,w.sellEvents,w.uniqueBuyers,w.uniqueSellers,w.estimatedBuySol,w.estimatedSellSol,w.estimatedNetSol,w.buySellEventRatio ?? "",w.proxyMomentumPct ?? "");
    }
    mintLines.push(row.map(csvEscape).join(","));
  }
  fs.writeFileSync(mintCsvPath, mintLines.join("\n"));

  const eventLines = [["target_mint","target_buy_time","target_buy_signature","target_outcome","event_time","lead_sec","wallet","direction","token_amount","estimated_sol","price_proxy_sol_per_token","venue","signature","slot"].join(",")];
  for (const e of events) eventLines.push([
    e.targetMint,new Date(e.targetBuyTime*1000).toISOString(),e.targetBuySignature,e.targetOutcome,new Date(e.blockTime*1000).toISOString(),e.leadSec,e.wallet,e.direction,e.tokenAmount,e.estimatedSol ?? "",e.priceProxySolPerToken ?? "",e.venue,e.signature,e.slot
  ].map(csvEscape).join(","));
  fs.writeFileSync(eventCsvPath, eventLines.join("\n"));

  const lines: string[] = [];
  lines.push("BOT PROFILER V4 - PRE-BUY WINDOW FORENSICS");
  lines.push("==========================================");
  lines.push(`Target: ${TARGET_WALLET}`);
  lines.push(`Analyzed target entries: ${mints.length}`);
  lines.push(`Pre-buy window: ${PROFILE_PREBUY_WINDOW_SEC}s`);
  lines.push("");
  lines.push("MEDIAN FLOW BEFORE TARGET BUY");
  for (const sec of [10,30,60,180]) {
    lines.push(`${String(sec).padStart(3)}s: buys=${medianWindow(mints,sec,"buyEvents") ?? "n/a"} sells=${medianWindow(mints,sec,"sellEvents") ?? "n/a"} uniqBuyers=${medianWindow(mints,sec,"uniqueBuyers") ?? "n/a"} netSOL=${medianWindow(mints,sec,"estimatedNetSol")?.toFixed(4) ?? "n/a"} proxyMomentum=${medianWindow(mints,sec,"proxyMomentumPct")?.toFixed(2) ?? "n/a"}%`);
  }
  lines.push("");
  lines.push("TOP WALLETS IMMEDIATELY BEFORE TARGET");
  for (const s of stats.slice(0,30)) {
    lines.push(`${String(s.appearances180).padStart(3)}x180  ${String(s.appearances60).padStart(3)}x60  ${String(s.appearances30).padStart(3)}x30  ${String(s.appearances10).padStart(3)}x10  ${s.wallet}  lead=${fmtSec(s.medianClosestBuyLeadSec)}  W/L=${s.wins}/${s.losses}  net+win=${s.netPositiveWindowCount}  base1h=${s.baselineTx1h ?? "n/a"} base24h=${s.baselineTx24h == null ? "n/a" : `${s.baselineTx24h}${s.baseline24hLowerBound ? "+" : ""}`}`);
  }
  lines.push("");
  lines.push("INTERPRETATION NOTES");
  lines.push("- A wallet that repeats in the 10-30s columns is much more interesting than one seen only somewhere in the prior 180s.");
  lines.push("- High baseline activity can make recurrence less specific; a wallet with thousands of unrelated transactions is a weaker alpha clue.");
  lines.push("- estimated SOL and price proxy include fees/tips and are directional heuristics, not exact execution prices.");
  lines.push("- Public on-chain recurrence is not proof of insider status, common ownership, or non-public information.");
  fs.writeFileSync(txtPath, lines.join("\n"));

  return { jsonPath, txtPath, walletCsvPath, mintCsvPath, eventCsvPath };
}
...
async function main() {
  ...
  console.log(`Entries to analyze:      ${PROFILE_PREBUY_POSITIONS}`);
  console.log(`Pre-buy window:          ${PROFILE_PREBUY_WINDOW_SEC} sec`);
  console.log(`Sig limit / mint:        ${PROFILE_PREBUY_SIG_LIMIT}`);
  console.log(`Baseline wallets:        ${PROFILE_BASELINE_WALLETS}`);
  ...
  console.log("[2/6] Fetching target transaction bodies...");
  const txMap = await getTransactions(sigInfos.map((x) => x.signature));
  console.log(`[OK] ${txMap.size} transaction bodies.`);

  console.log("[3/6] Parsing target Pump.fun / PumpSwap positions...");
  const trades = parseTargetTrades(sigInfos, txMap);
  const positions = buildPositions(trades).filter((p) => p.firstBuy).slice(0, PROFILE_PREBUY_POSITIONS);
  console.log(`[OK] ${trades.length} Pump trades; analyzing ${positions.length} recent first entries.`);

  console.log(`[4/6] Scanning ${PROFILE_PREBUY_WINDOW_SEC}s immediately BEFORE each target BUY...`);
  const mintSummaries: PreBuyMint[] = [];
  const allEvents: PreBuyEvent[] = [];
  for (let i = 0; i < positions.length; i++) {
    const p = positions[i];
    console.log(`\n[WINDOW ${i + 1}/${positions.length}] ${p.mint}`);
    try {
      const r = await scanOnePreBuy(p);
      mintSummaries.push(r.summary);
      allEvents.push(...r.events);
      const w10=r.summary.windows["10"], w30=r.summary.windows["30"], w60=r.summary.windows["60"], w180=r.summary.windows["180"];
      console.log(`  outcome: ${p.outcome}${p.estimatedRoiPct == null ? "" : ` (${p.estimatedRoiPct >= 0 ? "+" : ""}${p.estimatedRoiPct.toFixed(1)}% est.)`}  hold=${fmtSec(p.holdSeconds)}`);
      console.log(`  10s:  B/S=${w10.buyEvents}/${w10.sellEvents} uniqB=${w10.uniqueBuyers} netSOL=${w10.estimatedNetSol.toFixed(4)} momentum=${w10.proxyMomentumPct == null ? "n/a" : `${w10.proxyMomentumPct.toFixed(1)}%`}`);
      console.log(`  30s:  B/S=${w30.buyEvents}/${w30.sellEvents} uniqB=${w30.uniqueBuyers} netSOL=${w30.estimatedNetSol.toFixed(4)} momentum=${w30.proxyMomentumPct == null ? "n/a" : `${w30.proxyMomentumPct.toFixed(1)}%`}`);
      console.log(`  60s:  B/S=${w60.buyEvents}/${w60.sellEvents} uniqB=${w60.uniqueBuyers} netSOL=${w60.estimatedNetSol.toFixed(4)}`);
      console.log(`  180s: B/S=${w180.buyEvents}/${w180.sellEvents} uniqB=${w180.uniqueBuyers} netSOL=${w180.estimatedNetSol.toFixed(4)} scanned=${r.summary.priorSignaturesScanned}${r.summary.truncated ? "+" : ""}`);
      console.log(`  closest buyers: ${r.summary.topBuyers180.slice(0,5).map((x)=>`${short(x.wallet)}@${x.closestLeadSec.toFixed(0)}s`).join(", ") || "none"}`);
    } catch (e: any) {
      console.log(`  [WARN] window scan failed: ${e?.message || e}`);
    }
  }

  let stats = buildPreBuyWalletStats(mintSummaries, allEvents);
  await enrichBaseline(stats);

  console.log("[6/6] Writing V4 reports...");
  const files = writeV4Reports(mintSummaries, allEvents, stats, trades);

  console.log("\n============================================================");
  console.log("                   PRE-BUY FINGERPRINT");
  console.log("============================================================");
  console.log(`Analyzed target entries:        ${mintSummaries.length}`);
  console.log(`Extracted pre-buy events:       ${allEvents.length}`);
  console.log(`Median hold:                    ${fmtSec(median(positions.map((p)=>p.holdSeconds).filter((x): x is number => x != null)))}`);
  console.log("\nMedian flow before target:");
  for (const sec of [10,30,60,180]) {
    console.log(`  ${String(sec).padStart(3)}s  buys=${medianWindow(mintSummaries,sec,"buyEvents") ?? "n/a"} sells=${medianWindow(mintSummaries,sec,"sellEvents") ?? "n/a"} uniqBuyers=${medianWindow(mintSummaries,sec,"uniqueBuyers") ?? "n/a"} netSOL=${medianWindow(mintSummaries,sec,"estimatedNetSol")?.toFixed(4) ?? "n/a"}`);
  }

  console.log("\nTop wallets immediately before target:");
  if (!stats.length) console.log("  none found");
  for (const s of stats.slice(0,20)) {
    console.log(`  180=${String(s.appearances180).padStart(2)} 60=${String(s.appearances60).padStart(2)} 30=${String(s.appearances30).padStart(2)} 10=${String(s.appearances10).padStart(2)}  ${s.wallet}  lead=${fmtSec(s.medianClosestBuyLeadSec)}  base1h=${s.baselineTx1h ?? "n/a"} base24h=${s.baselineTx24h == null ? "n/a" : `${s.baselineTx24h}${s.baseline24hLowerBound ? "+" : ""}`}`);
  }
